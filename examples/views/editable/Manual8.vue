<template>
  <div v-loading="loading">
    ...
    <el-editable
      ...
      <el-editable-column prop="nested.nested[0]" label="嵌套" min-width="160" show-overflow-tooltip :edit-render="{type: 'default'}">
        <template v-slot="{ row }">
          {{row.nested.nested[0]}}
        </template>
        <template v-slot:edit="scope">
          <el-input size="mini" v-model="scope.row.nested.nested[0]" @input="$refs.editable.updateStatus(scope)"></el-input>
        </template>
      </el-editable-column>
...
export default {
  data () {
    return {
      ...
      validRules: {
        'nested.nested[0]': [
          { required: true, message: '请输入嵌套字段', trigger: 'change' },
          { min: 3, max: 50, message: '嵌套字段长度 4-20 个字符', trigger: 'change' }
        ],
        name: [
